...
$log  = Join-Path $root '.vs4_progress.log'
...
Add-Content -LiteralPath $log -Value ("=== plus(cap=2) vs 原版 (2026-09-16) start {0} ===" -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'))
...
foreach ($job in $jobs) {
    ...
    $arch = Join-Path $main ("Evaluation\vs4_throttle_{0}_{1}" -f $job.name, $stamp)
